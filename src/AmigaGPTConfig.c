...
struct AmigaGPTConfigData {
    ...
    /* MorphOS view/debug/window settings */
...
static void setDefaults(struct AmigaGPTConfigData *data) {
    ...
    data->markdownFormatting = TRUE;
    data->chatFontSize = CHAT_OUTPUT_FONT_SIZE_DEFAULT;
    data->chatLineWrap = TRUE;
    data->debugStreamLog = FALSE;
    data->debugLifecycleLog = FALSE;
    data->mainWindowLeft = 0;
    data->mainWindowTop = 0;
    data->mainWindowWidth = 0;
    data->mainWindowHeight = 0;
...
SAVEDS ULONG mConfigGet(struct IClass *cl, Object *obj, struct opGet *msg) {
    ...
    case MUIA_AmigaGPTConfig_FixedWidthFonts:
        *store = data->fixedWidthFonts;
        return TRUE;
    case MUIA_AmigaGPTConfig_MarkdownFormatting:
        *store = data->markdownFormatting;
        return TRUE;
    case MUIA_AmigaGPTConfig_ChatFontSize:
        *store = data->chatFontSize;
        return TRUE;
    case MUIA_AmigaGPTConfig_ChatLineWrap:
        *store = data->chatLineWrap;
        return TRUE;
    case MUIA_AmigaGPTConfig_DebugStreamLog:
        *store = data->debugStreamLog;
        return TRUE;
    case MUIA_AmigaGPTConfig_DebugLifecycleLog:
        *store = data->debugLifecycleLog;
        return TRUE;
    case MUIA_AmigaGPTConfig_MainWindowLeft:
        *store = (ULONG)data->mainWindowLeft;
        return TRUE;
    case MUIA_AmigaGPTConfig_MainWindowTop:
        *store = (ULONG)data->mainWindowTop;
        return TRUE;
    case MUIA_AmigaGPTConfig_MainWindowWidth:
        *store = data->mainWindowWidth;
        return TRUE;
    case MUIA_AmigaGPTConfig_MainWindowHeight:
        *store = data->mainWindowHeight;
        return TRUE;
...
SAVEDS ULONG mConfigSet(struct IClass *cl, Object *obj, struct opSet *msg) {
    ...
    for (tags = msg->ops_AttrList; (tag = NextTagItem(&tags));) {
        ...
        case MUIA_AmigaGPTConfig_FixedWidthFonts:
            ...
            break;
        case MUIA_AmigaGPTConfig_MarkdownFormatting:
            if (data->markdownFormatting != (ULONG)ti_Data) {
                data->markdownFormatting = (ULONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_ChatFontSize:
            if (data->chatFontSize != (ULONG)ti_Data) {
                data->chatFontSize = configClampChatFontSize((ULONG)ti_Data);
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_ChatLineWrap:
            if (data->chatLineWrap != (ULONG)ti_Data) {
                data->chatLineWrap = (ULONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_DebugStreamLog:
            if (data->debugStreamLog != (ULONG)ti_Data) {
                data->debugStreamLog = (ULONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_DebugLifecycleLog:
            if (data->debugLifecycleLog != (ULONG)ti_Data) {
                data->debugLifecycleLog = (ULONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_MainWindowLeft:
            if (data->mainWindowLeft != (LONG)ti_Data) {
                data->mainWindowLeft = (LONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_MainWindowTop:
            if (data->mainWindowTop != (LONG)ti_Data) {
                data->mainWindowTop = (LONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_MainWindowWidth:
            if (data->mainWindowWidth != (ULONG)ti_Data) {
                data->mainWindowWidth = (ULONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_MainWindowHeight:
            if (data->mainWindowHeight != (ULONG)ti_Data) {
                data->mainWindowHeight = (ULONG)ti_Data;
                changed = TRUE;
            }
            break;
        case MUIA_AmigaGPTConfig_UserTextAlignment:
...
static LONG saveConfig(struct AmigaGPTConfigData *data) {
    ...
    json_object_object_add(configJsonObject, "xaiTTSVoice",
                           json_object_new_int(data->xaiTTSVoice));
    ...
    json_object_object_add(
        ...
        json_object_new_boolean((BOOL)data->markdownFormatting));
    json_object_object_add(
        configJsonObject, "chatLineWrap",
        json_object_new_boolean((BOOL)data->chatLineWrap));
    json_object_object_add(configJsonObject, "chatFontSize",
                           json_object_new_int((int)data->chatFontSize));
    json_object_object_add(
        configJsonObject, "debugStreamLog",
        json_object_new_boolean((BOOL)data->debugStreamLog));
    json_object_object_add(
        configJsonObject, "debugLifecycleLog",
        json_object_new_boolean((BOOL)data->debugLifecycleLog));
    json_object_object_add(configJsonObject, "mainWindowLeft",
                           json_object_new_int(data->mainWindowLeft));
    json_object_object_add(configJsonObject, "mainWindowTop",
                           json_object_new_int(data->mainWindowTop));
    json_object_object_add(configJsonObject, "mainWindowWidth",
                           json_object_new_int((int)data->mainWindowWidth));
    json_object_object_add(configJsonObject, "mainWindowHeight",
                           json_object_new_int((int)data->mainWindowHeight));
#endif
    ...
    if (Write(file, configJsonString, configJsonLen) != (LONG)configJsonLen) {
...
static LONG loadConfig(struct AmigaGPTConfigData *data) {
    ...
    if (json_object_object_get_ex(configJsonObject, "xaiTTSVoice", &valueObj))
        data->xaiTTSVoice = json_object_get_int(valueObj);

    if (json_object_object_get_ex(configJsonObject, "xaiTTSVoiceId",
                                  &valueObj)) {
    ...
    if (json_object_object_get_ex(configJsonObject, "markdownFormatting",
                                  &valueObj))
        data->markdownFormatting =
            (ULONG)json_object_get_boolean(valueObj);
    else
        data->markdownFormatting = TRUE;

    if (json_object_object_get_ex(configJsonObject, "chatLineWrap", &valueObj))
        data->chatLineWrap = (ULONG)json_object_get_boolean(valueObj);
    else
        data->chatLineWrap = TRUE;

    if (json_object_object_get_ex(configJsonObject, "chatFontSize", &valueObj))
        data->chatFontSize = configClampChatFontSize(
            (ULONG)json_object_get_int(valueObj));
    else
        data->chatFontSize = CHAT_OUTPUT_FONT_SIZE_DEFAULT;
    ...
    if (json_object_object_get_ex(configJsonObject, "debugStreamLog",
                                  &valueObj))
        data->debugStreamLog =
            json_object_get_boolean(valueObj) ? TRUE : FALSE;

    if (json_object_object_get_ex(configJsonObject, "debugLifecycleLog",
                                  &valueObj))
        data->debugLifecycleLog =
            json_object_get_boolean(valueObj) ? TRUE : FALSE;

    if (json_object_object_get_ex(configJsonObject, "mainWindowLeft",
                                  &valueObj) &&
        json_object_is_type(valueObj, json_type_int))
        data->mainWindowLeft = json_object_get_int(valueObj);

    if (json_object_object_get_ex(configJsonObject, "mainWindowTop",
                                  &valueObj) &&
        json_object_is_type(valueObj, json_type_int))
        data->mainWindowTop = json_object_get_int(valueObj);

    if (json_object_object_get_ex(configJsonObject, "mainWindowWidth",
                                  &valueObj) &&
        json_object_is_type(valueObj, json_type_int))
        data->mainWindowWidth = (ULONG)json_object_get_int(valueObj);

    if (json_object_object_get_ex(configJsonObject, "mainWindowHeight",
                                  &valueObj) &&
        json_object_is_type(valueObj, json_type_int))
        data->mainWindowHeight = (ULONG)json_object_get_int(valueObj);

    if (data->mainWindowWidth < MAIN_WINDOW_SAVED_MIN_WIDTH ||
        data->mainWindowHeight < MAIN_WINDOW_SAVED_MIN_HEIGHT ||
        data->mainWindowWidth > 16384 || data->mainWindowHeight > 16384) {
        data->mainWindowLeft = 0;
        data->mainWindowTop = 0;
        data->mainWindowWidth = 0;
        data->mainWindowHeight = 0;
...
LONG writeConfig(void) {
    if (!configObj)
        return RETURN_ERROR;
    return DoMethod(configObj, MUIM_AmigaGPTConfig_Save);
}

LONG readConfig(void) {
    if (!configObj)
        return RETURN_ERROR;
    return DoMethod(configObj, MUIM_AmigaGPTConfig_Load);
}
...
void configGetSpeechRequestSettings(struct SpeechRequestSettings *out) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        struct json_object *xKey = json_object_object_get(p, "xaiApiKey");
        if (xKey != NULL) {
            if (out->xaiApiKey != NULL) {
                FreeVec(out->xaiApiKey);
                out->xaiApiKey = NULL;
            }
            out->xaiApiKey = dupStrCfg(json_object_get_string(xKey));
        }
        struct json_object *xVoice = json_object_object_get(p, "xaiTTSVoice");
        if (xVoice != NULL)
            out->xaiVoice = (XAITTSVoice)json_object_get_int(xVoice);
        struct json_object *xVoiceId =
            json_object_object_get(p, "xaiTTSVoiceId");
        if (xVoiceId != NULL) {
            CONST_STRPTR vid = json_object_get_string(xVoiceId);
...
XAITTSVoice configGetXAITTSVoice(void) {
    XAITTSVoice val = XAI_TTS_VOICE_EVE;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_XAITTSVoice, &val);
    return val;
}

void configSetXAITTSVoice(XAITTSVoice value) {
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_XAITTSVoice, value);
}

STRPTR configGetXAITTSVoiceId(void) {
    STRPTR val = NULL;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_XAITTSVoiceId, &val);
    return val;
}
...
ULONG configGetMarkdownFormatting(void) {
#ifdef __MORPHOS__
    ULONG val = TRUE;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_MarkdownFormatting, &val);
    return val;
#else
    return TRUE;
#endif
...
void configSetMarkdownFormatting(ULONG value) {
#ifdef __MORPHOS__
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_MarkdownFormatting, value);
#else
    (void)value;
#endif
}

ULONG configGetChatLineWrap(void) {
#ifdef __MORPHOS__
    ULONG val = TRUE;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_ChatLineWrap, &val);
    return val;
#else
    return TRUE;
#endif
...
void configSetChatLineWrap(ULONG value) {
#ifdef __MORPHOS__
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_ChatLineWrap, value);
#else
    (void)value;
#endif
}

ULONG configGetChatFontSize(void) {
#ifdef __MORPHOS__
    ULONG val = CHAT_OUTPUT_FONT_SIZE_DEFAULT;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_ChatFontSize, &val);
    return val;
#else
    return CHAT_OUTPUT_FONT_SIZE_DEFAULT;
#endif
}

void configSetChatFontSize(ULONG value) {
#ifdef __MORPHOS__
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_ChatFontSize,
            configClampChatFontSize(value));
#else
    (void)value;
#endif
}
...
LONG configGetMainWindowLeft(void) {
    ULONG val = 0;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_MainWindowLeft, &val);
    return (LONG)val;
}

void configSetMainWindowLeft(LONG value) {
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_MainWindowLeft, (ULONG)value);
}

LONG configGetMainWindowTop(void) {
    ULONG val = 0;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_MainWindowTop, &val);
    return (LONG)val;
}

void configSetMainWindowTop(LONG value) {
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_MainWindowTop, (ULONG)value);
}

ULONG configGetMainWindowWidth(void) {
    ULONG val = 0;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_MainWindowWidth, &val);
    return val;
}

void configSetMainWindowWidth(ULONG value) {
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_MainWindowWidth, value);
}

ULONG configGetMainWindowHeight(void) {
    ULONG val = 0;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_MainWindowHeight, &val);
    return val;
}

void configSetMainWindowHeight(ULONG value) {
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_MainWindowHeight, value);
}

ULONG configGetDebugStreamLog(void) {
    ULONG val = FALSE;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_DebugStreamLog, &val);
    return val;
}

void configSetDebugStreamLog(ULONG value) {
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_DebugStreamLog, value);
}

ULONG configGetDebugLifecycleLog(void) {
    ULONG val = FALSE;
    if (configObj)
        get(configObj, MUIA_AmigaGPTConfig_DebugLifecycleLog, &val);
    return val;
}

void configSetDebugLifecycleLog(ULONG value) {
    if (configObj)
        set(configObj, MUIA_AmigaGPTConfig_DebugLifecycleLog, value);
}
