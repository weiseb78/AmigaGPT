...
- **FlexCat catalogs (HARD RULE)** — FlexCat ignores gettext `#, fuzzy` and would ship bad/empty `msgstr` values. Never leave fuzzy or empty `msgstr` in `catalogs/german/deutsch.po` (`tools/flexcat-po-prepare.sh` hard-fails). Only actively translate German; other `.po` files stay `msgstr` = `msgid`. Cursor rule: `.cursor/rules/flexcat-catalog-policy.mdc`. Details: `docs/README.md` § Katalog.
