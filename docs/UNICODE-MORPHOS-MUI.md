...
**CJK (Chinesisch/Japanisch/Koreanisch):** CJK-Zeichen werden in der Chat-Scintilla als □ (Tofu) angezeigt, da DejaVu keine CJK-Glyphen enthält. DejaVu Sans/Mono über `SCI_STYLESETFONT` (§2, Code-Viewer/Chat) bleibt unverändert; **HW-Tests (2026-08)** zeigten jedoch, dass ein Wechsel auf einen CJK-fähigen TTEngine-Font (WenQuanYi Micro Hei — per Style, als Default oder per Dateipfad) **keine lesbaren CJK-Glyphen** liefert. Style-Segmentierung mit separatem CJK-Font ist damit **nicht umsetzbar**; der Code für `utf8_augment_cjk_styles()` / `CHAT_OUTPUT_STYLE_CJK` wurde entfernt. Erkennung umfasst BMP und supplementary CJK (z. B. Extension B `U+20000` als 4-Byte-UTF-8) via `utf8_contains_cjk()`. Live-Scintilla-Refresh während des Streams mit CJK kann MorphOS/TTEngine **einfrieren** — der Fork überspringt deshalb Live-Redraw und Netz-TTS bei erkanntem CJK; Anzeige einmal nach Stream-Ende. **Export** enthält korrektes UTF-8 einschließlich CJK. **MarkdownView.mcc** (eigene MUI-Klasse, TTEngine-Fallback wie in MarkdownEdit-Preview) könnte als Chat-Backend lesbare CJK-Darstellung ermöglichen.
